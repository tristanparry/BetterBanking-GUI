...
public class AccessAccount_GUI implements ActionListener {
	 ...
	 */
	public AccessAccount_GUI(Client clientArg) {
		this.c = clientArg;
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == this.chequingAccountButton) {
			...
			clientChequingAccount = new Chequing(Utilities.connectToMongoDB().getCollection("accountFiles").find(new Document("Username", c.getUsername()).append("AccountType", "Chequing")).first());
		...
		if (ae.getSource() == this.savingsAccountButton) {
			...
			clientSavingsAccount = new Savings(Utilities.connectToMongoDB().getCollection("accountFiles").find(new Document("Username", c.getUsername()).append("AccountType", "Savings")).first());
		...
		if (ae.getSource() == this.investmentAccountButton) {
			...
			clientInvestmentAccount = new Investment(Utilities.connectToMongoDB().getCollection("accountFiles").find(new Document("Username", c.getUsername()).append("AccountType", "Investment")).first());
	...
	public static void accessAccount(Client clientArg) {		
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				window = new AccessAccount_GUI(clientArg);
