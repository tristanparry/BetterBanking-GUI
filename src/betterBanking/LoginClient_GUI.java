	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == this.loginButton) {
			...
			if (tempUsername.isBlank() || tempPassword.isBlank()) {
				...
				try { usernameCheck = tempUsername.equals(Files.readAllLines(Paths.get("clientFiles/" + tempUsername + ".txt")).get(2)); } catch (IOException e) {}
				try { passwordCheck = tempPassword.equals(Files.readAllLines(Paths.get("clientFiles/" + tempUsername + ".txt")).get(3)); } catch (IOException e) {}
				
				if (usernameCheck && passwordCheck && this.loginAttempts <= 3) {
					// CREATE A CLIENT OBJECT
					Client newClient = null;
					try { newClient = new Client(Files.readAllLines(Paths.get("clientFiles/" + tempUsername + ".txt"))); } catch (IOException e) {}
					
					// TRANSITION + CALL MAIN METHOD AGAIN
					try { Thread.sleep(500); } catch (InterruptedException e1) {}
					this.frame.dispose();
					try { ClientOptions_GUI.clientOptions(newClient); } catch (IOException e) {}
